...
<head>
    ...
    <style>
        ...
        .status-Completed, .status-Delivered { color: #28a745; background: #e6f9e6; }
        .status-Cancelled { color: #d0011b; background: #ffe6e6; }
        .status-Pending, .status-Processing { color: #f39c12; background: #fff5e6; }

        .btn-action { border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer; transition: 0.2s; font-weight: bold; font-size: 13px;}
...
<div class="content">
        ...
        <div class="user-actions">
            <c:if test="${sessionScope.acc.role == 'Admin'}">
                <form action="user-manager" method="POST" style="margin: 0;">
                    <input type="hidden" name="action" value="toggleLockDetail">
                    <input type="hidden" name="userId" value="${user.id}">
                    <input type="hidden" name="currentStatus" value="${user.active}">

                    <c:choose>
                        <c:when test="${user.active}">
                            <button type="submit" class="btn-action btn-lock" onclick="return confirm('Bạn chắc chắn muốn KHÓA tài khoản này?');">
                                <i class="fa-solid fa-lock"></i> Khóa tài khoản
                            </button>
                        </c:when>
                        <c:otherwise>
                            <button type="submit" class="btn-action btn-unlock" onclick="return confirm('Bạn muốn MỞ KHÓA tài khoản này?');">
    ...
    <c:if test="${not empty param.msg}">
        <div style="padding: 12px; margin-bottom: 20px; border-radius: 4px; font-weight: bold; background: #d4edda; color: #155724; border: 1px solid #c3e6cb;">
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card" style="border-top-color: ${(stats.totalOrders > 0 && (stats.cancelledCount / stats.totalOrders) > 0.3) ? '#d0011b' : '#28a745'}">
            <h4>Tỷ lệ hủy đơn</h4>
            <div class="value">
                <c:choose>
        ...
        <div class="profile-card">
            ...
            <div style="text-align: center; margin-top: 5px;">
                <span class="status-badge status-${user.active ? 'Completed' : 'Cancelled'}">
                    ${user.active ? 'Đang hoạt động' : 'Đang bị khóa'}
                </span>
            </div>
                ...
                <table>
                    ...
                    <c:forEach items="${userOrders}" var="o">
                        <tr>
                            ...
                            <td>
                                <a href="order-detail?id=${o.orderId}" style="color: #1b6e76; font-size: 13px; font-weight: bold; text-decoration: none;">Chi tiết <i class="fa-solid fa-arrow-right-long"></i></a>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
